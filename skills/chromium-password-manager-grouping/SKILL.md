...
Use this skill to hand off or investigate Sol/Chromium password-manager grouping bugs where imported credentials are stored correctly but `chrome://password-manager/passwords` displays duplicate visible site rows, such as two `account.ycombinator.com` rows, or where the top-level password list shows an exact login subdomain instead of a Chrome-like grouped registrable domain such as `ycombinator.com`.
...
Each visible row represented one saved credential. The expected top-level behavior is one grouped site entry containing both credentials, with the list row displayed as `ycombinator.com` rather than `account.ycombinator.com` when Chromium's password-manager grouping/branding model can derive that registrable-domain label.

The important distinction is display versus storage: the top-level `chrome://password-manager/passwords` row may say `ycombinator.com`, but the stored credential must still retain the exact realm used for safe autofill matching, such as `https://account.ycombinator.com/`.
...
So the bug to hand off is: **Sol's Chromium password-manager list model/WebUI is not grouping multiple local-store credentials that share the same normalized `origin_url`/`signon_realm` into a single top-level site row, and that top-level row is not using the expected grouped-domain display label such as `ycombinator.com`.**

Do not "fix" this by rewriting stored credential origins from `account.ycombinator.com` to `ycombinator.com`; that would change autofill matching scope. The fork should derive `ycombinator.com` as a top-level display/grouping label in the password-manager list model or WebUI, not by mutating credential storage origins.
...
- Sol client import should only normalize to the exact origin realm, for example `https://account.ycombinator.com/`. It should not rewrite credentials to eTLD+1, for example `https://ycombinator.com/`, because that can broaden autofill scope and change security semantics. The desired `ycombinator.com` string belongs in the password-manager top-level display/grouping layer.
...
- **Chromium fork issue:** stored rows are already normalized to the same `origin_url`/`signon_realm`, but `chrome://password-manager/passwords` renders separate site rows instead of one top-level grouped row with multiple credentials, or displays that top-level row as `account.domain.com` instead of the expected grouped domain such as `domain.com`.
...
Can the fork expose or restore the Chromium grouping path used by `chrome://password-manager/passwords` so multiple credentials with the same normalized realm render as one top-level site row with multiple accounts, and so that row displays a grouped-domain label such as `ycombinator.com` instead of the login subdomain `account.ycombinator.com`, while still preserving exact-origin autofill semantics?
...
- `chrome://password-manager/passwords` shows multiple credentials for the same normalized realm as one grouped top-level site entry.
- The grouped top-level site entry displays the expected grouped-domain label, for example `ycombinator.com` instead of `account.ycombinator.com`, when Chromium's password-manager model has enough information to derive it.
